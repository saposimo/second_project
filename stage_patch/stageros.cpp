...
public:
    ...
    base_last_globalpos_time_(0, 0, RCL_ROS_TIME)
  {
    ...
    // Stage 4 only paces the sim to wall-clock when the GUI runs. Headless it
    // free-runs, galloping /clock and breaking nav2 timing. realtime_factor>0
    // throttles the loop to (factor x) real time; <=0 keeps the old free-run.
    realtime_factor_ = this->declare_parameter<double>("realtime_factor", 1.0);
...
private:
  ...
  // Throttle the (otherwise free-running) Stage update loop to wall-clock time.
  // Called from the Stage thread before msg_lock_ is taken so a sleep here never
  // blocks incoming cmd_vel (handled on the executor thread).
  void pace_realtime()
  {
    if (realtime_factor_ <= 0.0) {
      return;  // pacing disabled: run as fast as possible (legacy behavior)
    }
    const uint64_t sim_usec = world_->SimTimeNow();
    if (sim_usec == 0) {
      return;
    }
    if (!have_pacing_start_) {
      pacing_wall_start_ = std::chrono::steady_clock::now();
      pacing_sim_start_usec_ = sim_usec;
      have_pacing_start_ = true;
      return;
    }
    const double sim_elapsed_s =
      static_cast<double>(sim_usec - pacing_sim_start_usec_) * 1e-6;
    const auto target = pacing_wall_start_ +
      std::chrono::duration_cast<std::chrono::steady_clock::duration>(
        std::chrono::duration<double>(sim_elapsed_s / realtime_factor_));
  ...
  bool have_pacing_start_ {false};
  std::chrono::steady_clock::time_point pacing_wall_start_;
  uint64_t pacing_sim_start_usec_ {0};
